...
fun Position.genMoves(): MoveList {
    val res = MoveList()
    val occ = colorBB[stm.color] or colorBB[(!stm).color]
    val targets = colorBB[stm.color].inv()
    ...
    if (ep != 0)
        for (from in pawns and pawnAttackBB(!stm, ep))
    ...
    when (stm) {
        WHITE -> {
            if ((cr and WHITE_OO) > 0 && noBlocks(F1, G1) && !sqAttacked(F1, !stm))
                res.add(Move(E1, G1, CASTLE))
            if ((cr and WHITE_OOO) > 0 && noBlocks(B1, C1, D1) && !sqAttacked(D1, !stm))
                res.add(Move(E1, C1, CASTLE))
        }
        BLACK -> {
            if ((cr and BLACK_OO) > 0 && noBlocks(F8, G8) && !sqAttacked(F8, !stm))
                res.add(Move(E8, G8, CASTLE))
            if ((cr and BLACK_OOO) > 0 && noBlocks(B8, C8, D8) && !sqAttacked(D8, !stm))
